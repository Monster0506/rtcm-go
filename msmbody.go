...
func DecodeMSM(payload []byte) (*MSM, error) {
	...
	for i := range m.Satellites {
		m.Satellites[i].RoughRangeModMs = float64(r.ReadUint(10)) * twoPow(-10)
	}
	...
	if shape.hasFinePseudorange {
		for i := range m.Cells {
			...
			if shape.fineRangeExtended {
				v = float64(r.ReadInt(20)) * twoPow(-29)
			} else {
				v = float64(r.ReadInt(15)) * twoPow(-24)
			}
	...
	if shape.hasFinePhaserange {
		for i := range m.Cells {
			var v float64
			if shape.fineRangeExtended {
				v = float64(r.ReadInt(24)) * twoPow(-31)
			} else {
				v = float64(r.ReadInt(22)) * twoPow(-29)
			}
	...
	if shape.hasCNR {
		for i := range m.Cells {
			var v float64
			if shape.fineRangeExtended {
				v = float64(r.ReadUint(10)) * twoPow(-4)
			} else {
